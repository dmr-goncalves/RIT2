 ...
 * groupDB.java
 *
 * Class that supports a simple database of groups with three elements.
 *
 */
import java.io.File;
...
public class groupDB {
    ...
    public Properties pgroups = new Properties(); // Group data
    ...
    public groupDB(String _bdname) {
        ...
        for (Iterator it = pgroups.keySet().iterator(); it.hasNext();) {
     ...
     * Get biggest group
     */
    public int get_biggest_group() {
        int out = 0;
        int outfinal = 0;
        for (Iterator it = set.iterator(); it.hasNext();) {
            String group = (String) (it.next());
            
            out = pgroups.getProperty(group + ":n1", "").length();
            out += pgroups.getProperty(group + ":n2", "").length();
            out += pgroups.getProperty(group + ":n3", "").length();
            out += pgroups.getProperty(group + ":nam1", "").length();
            out += pgroups.getProperty(group + ":nam2", "").length();
            out += pgroups.getProperty(group + ":nam3", "").length();
            out += pgroups.getProperty(group, "").length();
